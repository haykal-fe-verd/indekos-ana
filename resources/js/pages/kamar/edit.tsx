...
import { FasilitasKamar, FotoKamar, Kamar, Kategori } from "@/types";
...
function PageKamarEdit({ kategori, kamar }: { kategori: Kategori[]; kamar: Kamar }) {
  //! hooks
  const {
    data,
    setData,
    ...
    kategori_id: kamar.kategori?.id ? kamar.kategori?.id.toString() : "",
    deskripsi_kamar: kamar.deskripsi_kamar,
    lokasi_kamar: kamar.lokasi_kamar,
    foto_kamar: [],
    fasilitas_kamar: [],
  });
  ...
  const [existingFoto, setExistingFoto] = React.useState<FotoKamar[]>([]);
  const [existingFasilitas, setExistingFasilitas] = React.useState<FasilitasKamar[]>([]);
  ...
  const handleFasilitasChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
  ...
  const handleRemoveExistingFoto = (item: FotoKamar) => {
    destroy(route("kamar.remove_foto", item.id));
    reset();
  };

  const handleRemoveExistingFasilitas = (item: FasilitasKamar) => {
    destroy(route("kamar.remove_fasilitas", item.id));
    reset();
  };
  ...
  const onSubmit: React.FormEventHandler = (e) => {
    ...
    post(route("kamar.update", kamar.id), {
      data: formData,
      onSuccess: () => reset(),
    });
  };
  ...
  React.useEffect(() => {
    ...
    if (kamar.fasilitas_kamar) {
      setExistingFasilitas(kamar.fasilitas_kamar);
    }
  }, [kamar.foto_kamar, kamar.fasilitas_kamar]);
      ...
      <Card className={cn("w-full border-none")}>
        <CardContent className={cn("p-6 space-y-4")}>
          ...
          <form onSubmit={onSubmit} className="w-full space-y-4">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="col-span-2 lg:col-span-1">
                ...
                <div className="flex flex-col gap-5">
                  <h1 className="text-2xl font-bold underline decoration-dashed decoration-primary">Foto Kamar</h1>
                  {existingFoto.map((item, index) => (
                        ...
                        {item.foto && (
                          <div className="w-full relative">
                            ...
                            <Button
                              ...
                              onClick={() => handleRemoveExistingFoto(item)}
                            >
                              <Trash2 />
                            </Button>
                          </div>
                ...
                <div className="flex flex-col gap-5 mt-10">
                  {foto.map((item, index) => (
                    <div key={index}>
                      ...
                      <div className="flex flex-col items-center space-x-2">
                        {imagePreviews[index] && (
                          <div className="">
                            <img src={imagePreviews[index]} alt={`Preview${index}`} className="w-full h-40" />
                          </div>
                        )}
                        <div className="flex flex-row items-center w-full space-x-2">
                          ...
                          />
                          <Button type="button" size="sm" variant="destructive" onClick={() => handleRemoveFoto(index)}>
                            <Trash2 />
                          </Button>
                        </div>
                      </div>
                  ...
                  <button type="button" className="p-2 text-white rounded-full w-fit bg-primary" onClick={addFoto}>
                    <PlusCircle />
                  </button>
                </div>
              </div>
              <div className="col-span-2 lg:col-span-1">
                {/* existing fasilitas */}
                <div className="flex flex-col gap-5">
                  <h1 className="text-2xl font-bold underline decoration-dashed decoration-primary">Fasilitas Kamar</h1>
                  {existingFasilitas.map((item, index) => (
                    <div key={index}>
                      <div className="flex items-center space-x-2">
                        <Input
                          type="text"
                          id={`existing_fasilitas_kamar${index}`}
                          name={`existing_fasilitas_kamar${index}`}
                          className="flex-1 mt-2"
                        ...
                        <Button
                          ...
                          size="sm"
                          variant="destructive"
                          onClick={() => handleRemoveExistingFasilitas(item)}
                        >
                          <Trash2 />
                        </Button>
                      </div>
                ...
                <div className="flex flex-col gap-5 mt-10">
                  ...
                  <button type="button" className="p-2 text-white rounded-full w-fit bg-primary" onClick={addFasilitas}>
                    <PlusCircle />
                  </button>
                </div>
              </div>
            ...
            <Button type="submit" disabled={processing} className={cn("inline-flex flex-row gap-3")}>
